...
def get_rnn_encode(embedding, cell, length_mask=None, scope=None, reuse=None):
    ...
    with tf.variable_scope(scope, 'RnnEncoding', reuse=reuse):
        ...
        step_encoded_input, encoded_input = tf.nn.dynamic_rnn(cell, embedding, sequence_length=length_mask, dtype=tf.float32)
        return step_encoded_input, encoded_input, cell.state_size
...
def get_bi_rnn_encode(embedding, f_cell, b_cell, length_mask=None, scope=None, reuse=None):
    ...
    with tf.variable_scope(scope, 'RnnEncoding', reuse=reuse):
        if length_mask is None:
            length_mask = tf.reduce_sum(tf.sign(tf.reduce_max(tf.abs(embedding), reduction_indices=2)),reduction_indices=1)
            length_mask = tf.to_int32(length_mask)
        step_encoded_input, encoded_input = tf.nn.bidirectional_dynamic_rnn(f_cell, b_cell, embedding, sequence_length=length_mask, dtype=tf.float32)
        encoded_input = tf.concat(encoded_input, 1)
        step_encoded_input = tf.concat(step_encoded_input, 2)
        return step_encoded_input, encoded_input, f_cell.state_size+b_cell.state_size
