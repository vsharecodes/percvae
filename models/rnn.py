...
def raw_rnn(cell, loop_fn,
  ...
  **NOTE: This method is still in testing, and the API may change.**
  ...
  with vs.variable_scope(scope or "rnn") as varscope:
    ...
    state_structure = cell.output_size
    flat_state_size = nest.flatten(state_structure)
    flat_state_dtypes = [flat_state[0].dtype] * len(flat_state_size)
    ...
    flat_state_ta = [
        tensor_array_ops.TensorArray(
            dtype=dtype_i, dynamic_size=True, size=0, name="rnn_state_%d" % i)
        for i, dtype_i in enumerate(flat_state_dtypes)]
    state_ta = nest.pack_sequence_as(structure=state_structure, flat_sequence=flat_state_ta)
    flat_zero_state = [
        array_ops.zeros(_concat(batch_size, size_i), dtype_i)
        for size_i, dtype_i in zip(flat_state_size, flat_state_dtypes)]
    zero_state = nest.pack_sequence_as(structure=state_structure, flat_sequence=flat_zero_state)
    ...
    def body(time, elements_finished, current_input,
             emit_ta, state, loop_state, state_ta):
      ...
      nest.assert_same_structure(state_ta, state)
      ...
      state_ta = nest.map_structure(
          lambda ta, emit: ta.write(time, state), state_ta, state)

      elements_finished = math_ops.logical_or(elements_finished, next_finished)

      return (next_time, elements_finished, next_input,
              emit_ta, next_state, loop_state, state_ta)
    ...
    (emit_ta, final_state, final_loop_state, state_ta) = returned[-4:]

    if init_loop_state is None:
      final_loop_state = None

    return (emit_ta, final_state, final_loop_state, state_ta)
